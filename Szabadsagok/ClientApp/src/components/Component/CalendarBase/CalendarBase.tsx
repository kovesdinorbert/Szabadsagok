...
const CalendarBase = forwardRef<ICalendarBase, any>((props : any, ref) => {
  ...
  const getEvents = () => {
    ...
    let start = moment(calendarRef.current.getApi().view.activeStart).format('LL');
  ...
  const dayRenderer = (date: any) => {
    let currentDay: YearConfigModel[] | undefined = yearData.filter(y => moment(y.date).isSame(moment(date.date), 'day'));
  ...
  useImperativeHandle(ref, () => ({
    changeDayType(dayConfig:YearConfigModel) {
      const currentDay = yearData.filter(y => moment(y.date).isSame(moment(dayConfig.date), 'day'));
    ...
    return (
      <div>
        {loaded ? 
        <FullCalendar selectable={props.selectable? props.selectable: true}
          ref={calendarRef} 
          locale={'hu'}
          headerToolbar={{
            right: 'prev,next',
